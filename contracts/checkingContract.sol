...
contract checkingContract {
    ...
    uint8 constant MAX_HANDLE_NAME_LENGTH = 16;
    uint8 constant MIN_HANDLE_NAME_LENGTH = 4;
    ...
    * @param handleName string length to be check
    */

    function checkAlphaNumeric(string memory handleName) public pure returns (bool){
    
    string memory VNinLowerCase = toLower(handleName);
    ...
    function isHandleNameValid (string memory _registrarName) internal pure returns (bool){
        ...
        require(length <= MAX_HANDLE_NAME_LENGTH && length >= MIN_HANDLE_NAME_LENGTH,"Handlename length should be between 4-16 characters");
        return true;
